...
class PlayState extends MusicBeatState
{
	...
	public var silence1:BGSprite;
	public var silence2:BGSprite;
	public var silence3:BGSprite;
	public var silence4:BGSprite;

	public var boyfriendGroup:FlxSpriteGroup;
	public var dadGroup:FlxSpriteGroup;
	public var gfGroup:FlxSpriteGroup;
	public var Stuff:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'house': //boo Week
				var bg:BGSprite = new BGSprite('NightSky', -600, -200, 0.9, 0.9);
				add(bg);
				bg.scale.set(1.6, 1.6);
				stuff = new BGSprite('DoorAnim', -1400, -550, 0.9, 0.9,['DoorCloseFast', 'DoorOpenSlow', 'DoorOpenFast', 'DoorClosed']);
				stuff.scale.set(2.5, 2.5);
				stuff.updateHitbox();
				add(stuff);
				silence1 = new BGSprite('Font1', -100, -100, 0.9, 0.9,['Font1']);
				silence1.scale.set(1.2, 1.2);
				silence1.scrollFactor.set(0, 0);
				silence1.alpha = 0;
				silence2 = new BGSprite('Font2', -100, -100, 0.9, 0.9,['Font2']);
				silence2.scrollFactor.set(0, 0);
				silence2.scale.set(1.2, 1.2);
				silence2.alpha = 0;
				silence3 = new BGSprite('Font3', -100, -100, 0.9, 0.9,['Font3']);
				silence3.scale.set(1.2, 1.2);
				silence3.scrollFactor.set(0, 0);
				silence3.alpha = 0;
				silence4 = new BGSprite('Font4', -100, -100, 0.9, 0.9,['Font4']);
				silence4.scale.set(1.2, 1.2);
				silence4.scrollFactor.set(0, 0);
				silence4.alpha = 0;
				

			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloween_bg', -200, -100, 0.9, 0.9, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curStage == 'house') {
                 stuff.animation.play('DoorClosed');
				}
	...
	override function beatHit()
	{
		...
		// events for boo
		if (Paths.formatToSongPath(SONG.song) == 'boo') {
			if(curBeat == 20) {
				stuff.animation.play('DoorOpenSlow');
			}
			if(curBeat == 26) {
			...
			if(curBeat == 130) {
				stuff.animation.play('DoorCloseFast');
			}
			if(curBeat == 148) {
				stuff.animation.play('DoorOpenFast');
			}
			if(curBeat == 149) {
				FlxTween.tween(dad,{alpha:1},0.5);
				FlxTween.tween(iconP2,{alpha:1},0.5);
